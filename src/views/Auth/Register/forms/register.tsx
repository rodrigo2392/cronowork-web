...
export default function LoginForm() {
  ...
  const doRegister = (values: registerRequest) => {
    login({email: values.email, password: values.password, name: values.name}, {
      onSuccess: () => {
        ...
        <Box
          ...
          />
          {
            error && <Alert severity="error">
              {(error as AxiosError)?.response?.status === 400 ? "Email o contraseña incorrecto" :"Ha ocurrido un error, inténtalo de nuevo más tarde"}
              </Alert>
          }
          ...
          <Box sx={{textAlign: 'left'}}>
            <TextField
              ...
              id="repeat_password"
            />
             {errors.repeat_password && <Typography color='error'>{errors.repeat_password.message}</Typography>}
          </Box>
